  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 720px) {
      ...
      .bottom-nav { display: grid; grid-template-columns: repeat(6, 1fr); position: fixed; left: 8px; right: 8px; bottom: calc(8px + env(safe-area-inset-bottom)); z-index: 20; background: rgba(255,255,255,.96); border: 1px solid var(--line); border-radius: 18px; box-shadow: var(--shadow); padding: 6px; gap: 4px; }
...
<body>
  ...
  <nav class="bottom-nav"><button class="active" data-tab="dashboard">Home</button><button data-tab="search">Search</button><button data-tab="recall">Recall</button><button data-tab="capture">Capture</button><button data-tab="write">Write</button><button data-tab="ops">Ops</button></nav>
  ...
  <script>
    ...
    function loadDefaultProject(project) { $('project').value = project; $('project-scope').value = 'project'; $('topic').value = ''; $('query').value = ''; $('target-type').value = 'fact'; localStorage.setItem('siqueira.project', project); localStorage.setItem('siqueira.projectScope', 'project'); runSearch(); loadTimeline(); loadCapture(); }
    function loadGlobalScope() { $('project-scope').value = 'global'; $('project').value = ''; $('topic').value = ''; $('query').value = ''; $('target-type').value = 'fact'; localStorage.setItem('siqueira.projectScope', 'global'); localStorage.setItem('siqueira.project', ''); runSearch(); loadTimeline(); loadCapture(); }
    function loadAllScope() { $('project-scope').value = 'all'; $('project').value = ''; $('topic').value = ''; $('query').value = ''; localStorage.setItem('siqueira.projectScope', 'all'); localStorage.setItem('siqueira.project', ''); runSearch(); loadTimeline(); loadCapture(); }
    ...
    async function loadCapture() { setStatus('capture-status','Loading…'); try { const body = await post('/v1/admin/capture', { profile_id:$('profile').value.trim() || 'default' }); setStatus('capture-status', `Mode ${body.mode}; ${body.global_memories} global memories.`, 'good'); const stats = [['Mode', body.mode], ['Target ratio', body.target_ratio], ['Raw turns today', body.raw_turns_saved_today], ['Chunks', body.chunks_created], ['Embeddings', body.embeddings_created], ['Facts', body.structured_facts], ['Decisions', body.structured_decisions], ['Skipped sensitive', body.skipped_sensitive], ['Global memories', body.global_memories]]; $('capture-stats').innerHTML = stats.map(([k,v]) => `<article class="item"><div class="small">${escapeHtml(k)}</div><h2>${escapeHtml(v)}</h2></article>`).join(''); renderHits('global-memory-results', body.recent_global_memories || []); } catch (err) { setStatus('capture-status', err.message, 'danger'); } }
    ...
    async function refreshAll() { await loadProjects(); await runSearch(); await loadTimeline(); await loadCapture(); }
