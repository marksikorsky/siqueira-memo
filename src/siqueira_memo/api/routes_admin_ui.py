...
<body>
    ...
    <section class="layout">
      ...
      <section class="card">
        ...
        <section id="search" class="panel">
          ...
          <div class="row"><div><label for="target-type">Type</label><select id="target-type"><option value="fact">Facts</option><option value="decision">Decisions</option><option value="message">Messages</option><option value="summary">Summaries</option></select></div><div><label for="status-filter">Status</label><select id="status-filter"><option value="">Any</option><option value="active">Active</option><option value="superseded">Superseded</option><option value="invalidated">Invalidated</option></select></div><div><label for="trust-filter">Trust</label><select id="trust-filter"><option value="any">Any</option><option value="high">High</option><option value="medium">Medium</option><option value="low_trust">Low trust</option><option value="conflicting">Conflicting</option><option value="stale">Stale</option><option value="unverified">Unverified</option></select></div></div>
  ...
  <script>
    ...
    function cardHtml(hit) { const type = hit.target_type || hit.kind; const title = hit.topic || hit.project || type; const trust = hit.trust_label ? `<span class="stat trust-badge" title="${escapeHtml(hit.trust_explanation || '')}">trust ${escapeHtml(hit.trust_label)} ${escapeHtml(hit.trust_score ?? '')}</span>` : ''; return `<article class="item" data-type="${escapeHtml(type)}" data-id="${escapeHtml(hit.id)}"><div class="item-head"><div><span class="badge">${escapeHtml(type)}</span> <strong>${escapeHtml(title)}</strong></div><span class="meta mono">${escapeHtml(hit.id)}</span></div><div class="preview">${escapeHtml(hit.preview || hit.statement || hit.decision || hit.title || '')}</div><div class="statline">${hit.project ? `<span class="stat">${escapeHtml(hit.project)}</span>` : ''}${hit.topic ? `<span class="stat">${escapeHtml(hit.topic)}</span>` : ''}${hit.status ? `<span class="stat">${escapeHtml(hit.status)}</span>` : ''}${trust}</div></article>`; }
